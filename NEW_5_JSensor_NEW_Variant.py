# -*- coding: utf-8 -*-
"""
Вставка сенсоров в C++ код для динамического анализа.
Защиты (не удалять):
  - Инструментируем только .cpp/.cc/.cxx/.c (не .h/.hpp) — иначе constexpr/шаблоны ломаются.
  - Пропускаем файлы из externals/, third_party/, vendor/ и т.п. (should_skip_external_library).
  - SAFE_SENSOR_MODE=True: сенсор только на вход в функцию (pasteSensor), без вставки в IF/FOR/WHILE.
  - id_file берём из path[2] (БД), не из enumerate.
"""
...
def _is_instrumentable_source_file(file_path):
    """
    Инструментируем только исходники компиляции (не заголовки).
    Это критично, чтобы не ломать constexpr/шаблоны/хедер-онли библиотеки
    и не получать 'SENSOR not declared' из заголовков.
    """
    if not file_path:
        return False
    p = file_path.lower()
    # только файлы, которые точно компилируются как TU
    return p.endswith(('.cpp', '.cc', '.cxx', '.c'))
...
def should_skip_external_library(file_path):
    """
    Проверяет, нужно ли пропустить файл как внешнюю библиотеку.
    Возвращает True, если файл находится в каталогах внешних библиотек.
    """
    if not file_path:
        return False
    ...
    # Список паттернов путей внешних библиотек (относительно PATH_FILE или абсолютные)
    exclude_patterns = [
        '/externals/',
        '/external/',
        '/third_party/',
        ...
        '/openssl/',
        '/curl/',
        # Системные пути (если попали в список)
        '/usr/include/',
        '/usr/local/include/',
        '/opt/',
        # CMake build каталоги
        '/build/',
        '/cmake-build/',
        '/out/',
        '/.git/',
    ...
    for pattern in exclude_patterns:
        if pattern in file_path_normalized:
            return True
    
    # Проверка: если файл находится вне PATH_FILE (если PATH_FILE задан)
    project_path = folder.get('PATH_FILE', '').strip()
    if project_path:
        try:
            project_path_normalized = os.path.normpath(project_path).replace('\\', '/')
            file_path_normalized_abs = os.path.abspath(file_path_normalized).replace('\\', '/')
            project_path_abs = os.path.abspath(project_path_normalized).replace('\\', '/')
            
            # Если файл не начинается с project_path, возможно это внешняя библиотека
            if not file_path_normalized_abs.startswith(project_path_abs):
                # Но не исключаем, если это просто другой каталог проекта
                # Исключаем только если это системные пути
                if file_path_normalized_abs.startswith('/usr/') or file_path_normalized_abs.startswith('/opt/'):
                    return True
        except Exception:
            # Если не удалось нормализовать путь, полагаемся только на паттерны выше
            pass
...
def sloy(list_f = get_list()):
    ...
    # SAFE режим: вставляем сенсор только на вход в функцию (через pasteSensor),
    # без массовой вставки в IF/FOR/WHILE по ccc_line_seg, чтобы не ломать код.
    SAFE_SENSOR_MODE = True
    ...
    for id_file, path in enumerate(list_f, start=1):
        ...
        if should_skip_external_library(path[0]):
            print("Пропущен внешний файл: {}".format(path[0]))
            continue

        # Не трогаем заголовки и прочие не-TU файлы
        if not _is_instrumentable_source_file(path[0]):
            continue

        # В БД реальный id_file лежит в path[2]; enumerate(id_file) использовать нельзя.
        db_file_id = path[2]
        ...
        # if id_file>1:
        #     break
        # if '/home/fokriz/Рабочий стол/Work/evrika/15ПК/src20210414/mts/sources_mts/lib' in path[0] or '/home/fokriz/Рабочий стол/Work/evrika/15ПК/src20210414/mts/sources_mts/bspscore' in path[0]:
        #     continue
        # if path[0][path[0].rfind('.'):] in ['.c','.h']:
        #     continue
        if DEPTH > 2 or SAFE_SENSOR_MODE:
            pasteSensFunc([db_file_id, path], getData(db_file_id, conn))
            continue
        print(id_file)
        FileNew =[]
        conc_include =''
